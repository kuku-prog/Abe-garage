        ...
        employeeService.getUserByEmail(employee_email, (err, results) => {
            if (err) {
                ...
                if (err) {
                    return res.status(500).json({
                        success: "false",
                        message: "Database connection error during registering employee",
                    });
                }
                else {
                    ...
                    employeeService.registerEmployeeInfo(req.body, (err, results) => {
                        if (err) {
                            return res.status(500).json({
                                success: "false",
                                message: "Database connection error during registering employee info",
                            });
                        }
                        else {
                            // password encryption
                            const salt = bcrypt.genSaltSync(10);
                            req.body.employee_password = bcrypt.hashSync(employee_password, salt);
                            employeeService.registerEmployeePassword(req.body, (err, results) => {
                                if (err) {
                                    return res.status(500).json({
                                        success: "false",
                                        message: "Database connection error during registering employee password",
                                    });
                                }
                                else {
